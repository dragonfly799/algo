...
public class HashTableBenchmark {
	...
	public static void main(String[] args) {
		HashTable<String, Integer> hashTable = new ChainedHashTable<>(100_000);
		int size = 1_000_000;
	...
	private static long fill(HashTable<String, Integer> hashTable, int size) {
		long start = System.currentTimeMillis();
		for (int i = 0; i < size; i++) {
			hashTable.put("k" + i, random.nextInt());
		}
	...
	private static long select(HashTable hashTable, int iterations, int size) {
		...
		for (int i = 0; i < iterations; i++) {
			String key = "k" + random.nextInt(size);
			hashTable.get(key);
		}
		long time = System.currentTimeMillis() - start;
